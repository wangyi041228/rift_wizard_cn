...
class StackLimit(Mutator):
    ...
    def __init__(self, stack_max):
        ...
        self.description = "每种物品至多持有 %d 个" % stack_max
...
class LairMultiplier(Mutator):
    ...
    def __init__(self, mult):
        Mutator.__init__(self)
        self.mult = mult
        self.description = "每关有 %d 倍的刷怪大门" % self.mult
...
class MonsterHPMult(Mutator):
    ...
    def __init__(self, mult=2):
        Mutator.__init__(self)
        self.mult = mult
        self.description = "所有敌人有 %d%% 的生命" % (self.mult * 100)
...
class EnemyShields(Mutator):
    ...
    def __init__(self, shields):
        ...
        self.description = "所有敌人额外获得 %d 点护盾" % self.shields
        self.global_triggers[EventOnUnitPreAdded] = self.on_enemy_added
...
class EnemyBuff(Mutator):
    ...
    def __init__(self, buff, exclude_named=None):
        Mutator.__init__(self)
        self.buff = buff
        self.description = "所有敌人获得 %s" % loc.dic.get(buff().name, buff().name)
...
class RandomSkillRestriction(Mutator):
    ...
    def __init__(self, chance=.5):
        Mutator.__init__(self)
        self.chance = chance
        self.description = "随机移除 %d%% 的被动" % (self.chance * 100)
...
class RandomSpellRestriction(Mutator):
    ...
    def __init__(self, chance=.5):
        Mutator.__init__(self)
        self.chance = chance
        self.description = "随机移除 %d%% 的咒语" % (self.chance * 100)
...
class SpellTagRestriction(Mutator):
    ...
    def __init__(self, tag):
        Mutator.__init__(self)
        self.tag = tag
        self.description = "只有 %s 咒语" % loc.dic.get(self.tag.name, self.tag.name)
...
class OnlySpell(Mutator):
    ...
    def __init__(self, spellname):
        ...
        self.description = "只能使用 %s 咒语" % loc.dic.get(self.spellname, self.spellname)
...
class SpawnWizards(Mutator):
    ...
    def __init__(self):
        Mutator.__init__(self)
        self.description = "第二关开始每关多一个敌方的巫师"
...
class ExtraElites(Mutator):
    ...
    def __init__(self, num):
        Mutator.__init__(self)
        self.num = num
        self.description = "每关随机额外多 %d 个高级怪物" % self.num
...
class SpellChargeMultiplier(Mutator):
    ...
    def __init__(self, mult):
        Mutator.__init__(self)
        self.mult = mult
        self.description = "每个法术有 %d%% 的充能上限" % (self.mult * 100)
...
class ExtraSpawns(Mutator):
    ...
    def __init__(self, spawn, num_extra):
        ...
        ex = spawn()
        self.description = "第二关开始每关多 %d 个 %s" % (self.num_extra, loc.dic.get(ex.name, ex.name))
...
def get_weekly_mutators():
    ...
    if spell_restriction_roll < .4:
        ...
        tag = r.choice(
            [Tags.Fire, Tags.Ice, Tags.Dark, Tags.Holy, Tags.Nature, Tags.Lightning, Tags.Sorcery, Tags.Conjuration,
             Tags.Enchantment])
