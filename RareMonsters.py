...
class UndeathIdolBuff(Buff):
	...
	def on_init(self):
		...
		self.description = "每回合治疗所有友方 [undead] 单位 1 点生命，对所有敌方 [living] 单位造成 1 点 [dark] 伤害"
...
class LifeIdolBuff(Buff):
	...
	def on_init(self):
		...
		self.description = "每回合治疗所有己方 [living] 单位 3 点生命"
...
class ClarityIdolBuff(Buff):
	...
	def on_init(self):
		...
		self.description = "每回合随机治愈一个友军的 [stun] 或 [berserk]"

	def get_tooltip(self):
		return "每回合随机治愈一个友军的 [stun]、[berserk] 或 [petrify]。"
...
class GrantSorcery(Spell):
	...
	def get_description(self):
		return "随机给一个友军远程 [Fire] 或 [Lightning] 攻击的能力，并对该元素免疫。"
...
class DarkPriestHeal(Spell):
	...
	def on_init(self):
		...
		self.cool_down = 3
		self.range = 0
		self.description = "治疗所有友军 15 点生命"
...
def Watcher():
	...
	unit.name = "Watcher"
	unit.description = "坚固且固定的单位，可进行远程 [lightning] 攻击"
	unit.max_hp = 40
...
class DreamerBuff(Buff):
	...
	def on_init(self):
		self.description = "每回合有 50% 的几率将所有其他单位传送走 4 格"
...
class WailOfPain(Spell):
	...
	def on_init(self):
		...
		self.damage = 22
		self.cool_down = 7
		self.description = "对 %d 格内的所有敌人造成 %d 点伤害" % (self.radius, self.damage)
...
class TreeThornSummon(Spell):
	...
	def get_description(self):
		return "在玩家旁召唤 %d 个仙灵荆棘" % self.num_thorns
...
class TimeKeeperTimestop(Spell):
	...
	def on_init(self):
		self.name = "Timestop"
		self.description = "[Stun] 所有敌人 1 回合"
		self.cool_down = 3
		self.range = 0
...
class BatBreath(BreathWeapon):
	...
	def get_description(self):
		return "吐出锥形的蝙蝠，对被占的地块造成 %d 点伤害，在空的地块召唤蝙蝠。" % self.damage
...
class JarAlly(Spell):
	...
	def get_description(self):
		return "一个非 [construct] 友军失去 10 点生命上限，灵魂瓶被摧毁之前不会死亡。"
...
class MedusaSnakeBuff(Buff):
	...
	def get_tooltip(self):
		return "受到伤害时召唤蛇。每受到 5 点伤害便召唤一条蛇，向上取整。"

class MedusaGaze(Spell):

	def on_init(self):
		self.name = "Medusa's Gaze"
		self.description = "石化视线内的所有单位 3 回合"
		self.cool_down = 6
		self.range = 0
...
def FlyTrap():
	...
	fly_summon.description = "召唤 3 个苍蝇群"
...
class WriteChaosScrolls(Spell):
	...
	def on_init(self):
		...
		self.description = "召唤 2 到 4 个 火焰活体卷轴或闪电活体卷轴"
		self.range = 0
		self.cool_down = 6
...
class BoxOfWoeBuff(Buff):
	...
	def on_init(self):
		...
		self.owner_triggers[EventOnDeath] = self.on_death
		self.description = "被摧毁时生成 3 个邪恶灵魂"
...
class JackolanternSpell(Spell):
	def on_init(self):
		...
		self.description = "至多随机 4 个友方的 [Dark] 单位获得 +1 伤害，持续 25 回合"
		self.range = 0
...
class WizardGrounding(Spell):
	...
	def on_init(self):
		...
		self.duration = 8
		self.description = "给施法者和友军 50%% 的 [Lightning] 抗性 %d 回合" % self.duration
		self.cool_down = 12
		self.range = 0
...
class WizardLightningFlash(Spell):
	...
	def on_init(self):
		...
		self.description = "传送施法者。致盲目的地视线内的所有单位 1 回合并造成伤害。"
		self.range = 20
...
class WizardStoneSkin(Spell):
	...
	def on_init(self):
		self.name = "Stoneskin"
		self.duration = 8
		self.description = "给施法者和友军 50%% 的 [Physical] 抗性 %d 回合" % self.duration
		self.cool_down = 12
		self.range = 0
...
def MaskWizard():
	...
	disperse.description = "传送单位至多 5 格"
...
def ArachnidWizard():
	...
	thorns.cool_down = 13
	thorns.description = "召唤荆棘植物包围一群单位"
	...
	voidorb.cool_down = 9
	voidorb.range = 18
	voidorb.description = "召唤一个缓慢移动的虚空法球"
	voidorb.damage = 7
...
class DarkBall(Spell):
	...
	def on_init(self):
		self.name = "Dark Ball"
		self.description = "造成 [dark] 和 [arcane] 伤害，熔化墙"
		self.radius = 2
		self.damage = 7
		self.cool_down = 3
...
class WizardBloodboil(Spell):
	...
	def on_init(self):
		...
		self.description = "视线内所有友军的 [physical] 和 [fire] 攻击临时获得 +4 点伤害"
		self.duration = 7
		self.cool_down = 4
		self.range = 0
...
def GlassWizard():
	...
	glassorb.range = 14
	glassorb.duration = 1
	glassorb.description = "召唤一个玻璃法球，璃化敌人，给友军护盾，抵达目标时对半径内造成 [physical] 伤害。"
...
class WizardChaosWord(Spell):
	...
	def on_init(self):
		...
		self.description = "将所有其他单位随机传送到新位置。击晕其中巫师之外的单位 1 回合。"
		self.duration = 1
		self.cool_down = 20
		self.range = 0
...
class WizardIceEye(Spell):
	def on_init(self):
		...
		self.description = "每回合随机对视线内的一个敌人造成 [ice] 伤害"
		self.cool_down = 25
		self.damage = 5
		self.duration = 9
		self.range = 0
...
def IceLich():
	...
	freezeball.description = "冻结 3 回合"
...
class WizardFireEye(Spell):
	def on_init(self):
		...
		self.description = "每回合随机对视线内的一个敌人造成 fire] 伤害"
		self.cool_down = 25
		self.damage = 5
...
class WizardIceAge(Spell):
	...
	def on_init(self):
		self.name = "Ice Age"
		self.description = "寒冰冲击在半径内冻结所有单位。\n治疗友军\n对敌人造成 [ice] 伤害。"
		self.duration = 4
		self.heal = 25
		self.damage = 8
		self.cool_down = 25
...
class WizardIcicle(Spell):
	...
	def on_init(self):
		self.name = "Icicle"
		self.description = "造成 [ice] 和 [physical] 伤害"
		self.damage = 4
		self.range = 6
...
class WizardIgnitePoison(Spell):
	...
	def on_init(self):
		...
		self.description = "目标中毒的单位失去全部层数的毒性，受到等量的 [fire] 伤害"
		self.range = 10
		self.cool_down = 3
...
def GoblinWizard():
	...
	sting.description = "施加 [poison] 7 回合"
...
class TwilightProtection(Spell):
	...
	def on_init(self):
		...
		self.duration = 8
		self.description = "给施法者和友军 50%% 的 [Dark] 和 [Holy] 抗性 %d 回合" % self.duration
		self.cool_down = 12
		self.range = 0
...
class FireProtection(Spell):
	...
	def on_init(self):
		...
		self.duration = 8
		self.description = "给施法者和友军 50%% 的 [Fire] 抗性 %d 回合" % self.duration
		self.cool_down = 12
		self.range = 0
...
class FrostfireProtection(Spell):
	...
	def on_init(self):
		...
		self.duration = 8
		self.description = "给施法者和友军 50%% 的 [Fire] 和 [Ice] 抗性 %d 回合" % self.duration
		self.cool_down = 12
		self.range = 0
...
class WizardIceGaze(Spell):
	...
	def on_init(self):
		...
		self.description = "冻结视线内的所有单位 3 回合"
		self.range = 0
		self.cool_down = 18
...
class WizardStarfireBeam(Spell):
	...
	def on_init(self):
		...
		self.description = "在束状内造成 [arcane] 伤害，对束状相邻地块造成 [fire] 伤害。熔化墙。"
		self.cool_down = 17
...
class StarfireOrb(Spells.OrbSpell):
	...
	def on_init(self):
		...
		self.description = ("生成一个缓慢移动的灼热法球"
							"\n\n每回合法球对视线内的所有敌人造成伤害。"
							"\n\n治疗视线内的所有友军。")
...
class TideOfSin(Spell):
	...
	def on_init(self):
		...
		self.description = "视线内的所有敌人受到伤害，随机失去一个咒语的一点充能。"
		self.damage = 1
		self.damage_type = Tags.Holy
		self.range = 0
		self.cool_down = 4
...
class TideOfRot(Spell):
	...
	def on_init(self):
		...
		self.description = "视线内的所有敌人失去 1 点生命上限，中毒 4 回合。"
		self.range = 0
		self.cool_down = 4
...
class RedRiderHealingBuff(Buff):
	...
	def on_init(self):
		self.description = "每当另一个单位受到 [physical] 或 [fire] 伤害时，治疗 5 点生命。"
...
class RedRiderBerserkingSpell(Spell):
	...
	def on_init(self):
		self.name = "Strife"
		self.description = "使视线内所有敌方召唤的单位狂暴 3 回合"
		self.cool_down = 13
		self.duration = 3
		self.range = 0
...
class ImpHarvestBuff(Buff):
	...
	def on_init(self):
		...
		self.description = "每当一个恶魔死亡时，倒计指示物减少 1"
...
def ImpCollector():
	...
	impspell.description = "随机召唤 2 个小鬼"

	harvest = WizardSelfBuff(ImpHarvestBuff, duration=5, cool_down=12)
	harvest.description = "每当一个恶魔死亡时，咒语倒计时减少 1 回合，持续 5 回合"
...
class CoatlBreath(BreathWeapon):
	...
	def get_description(self):
		return "吐息武器，使 [living] 敌人中毒，给友军护盾"
...
class ConstructShards(Buff):
	...
	def on_init(self):
		...
		self.global_triggers[EventOnDeath] = self.on_death
		self.description = "每当友方 [construct] 死亡时，在 4 格冲程内对至多三个敌人造成 6 点 [fire] 或 [physical] 伤害"
...
class GeminiCloneSpell(Spell):
	...
	def on_init(self):
		self.name = "Duplicate"
		self.description = ("召唤另一个生命等于自身的双生子。\n"
						    "只能同时存在两个双生子。")
		self.range = 0
...
def Thornface():
	...
	phasebolt.description = "将目标传送至多 3 格，在其旁召唤 3 个仙灵荆棘。"
...
class ArcanePhoenixBuff(Buff):
	...
	def get_tooltip(self):
		return "死亡时，对 6 格内的所有地块造成 25 点 [arcane] 伤害。友方单位获得 2 点护盾，而非受到伤害。熔化墙。"
...
class IdolOfSlimeBuff(Buff):
	...
	def on_init(self):
		...
		self.description = "每当非史莱姆的友军死去时，在原位召唤一个史莱姆。"
		self.global_triggers[EventOnDeath] = self.on_death
...
class CrucibleOfPainBuff(Buff):
	...
	def on_init(self):
		...
		self.description = "每累计造成 300 点伤害，便召唤一个熔炉猎犬。"
...
class FieryVengeanceBuff(Buff):
	...
	def on_init(self):
		self.global_triggers[EventOnDeath] = self.on_death
		self.description = "每当友军死亡时，随机对 3 格内的一个敌方单位造成 9 点 [fire] 伤害。"
...
class ConcussiveIdolBuff(Buff):
	...
	def on_init(self):
		self.global_triggers[EventOnDamaged] = self.on_damage
		self.description = "每当敌人受伤时，被击晕 1 回合。"
...
class VampirismIdolBuff(Buff):
	...
	def on_init(self):
		...
		self.global_triggers[EventOnDamaged] = self.on_damage
		self.description = "每当友军受伤时，治疗一半的生命。"
